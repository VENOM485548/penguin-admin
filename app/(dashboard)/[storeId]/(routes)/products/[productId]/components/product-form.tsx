...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const form = useForm<ProductFormValues, any, ProductFormValues>({
    ...
    defaultValues: initialData
      ? {
          ...initialData,
          // (optional) normalize images if needed:
  ...
  const onSubmit: SubmitHandler<ProductFormValues> = async (data) => {
    try {
      ...
      const payload = {
        ...data,
        // ensure images are only url objects
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <ImageUpload
                    multiple
                    value={field.value.map((img) => img.url)}  // ✅ convert objects to string[]
                    disabled={loading}
                    onChange={(urls) =>
                      field.onChange(urls.map((url) => ({ url })))  // ✅ convert string[] back to {url:string}[]
                    }
                    onRemove={(url) =>
